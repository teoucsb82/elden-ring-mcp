...
import { DEFAULT_DB_PATH, rerenderSections } from '../src/store/pages.js';
import { printDlcReport } from './report.js';

const db = openDb(process.env.ELDEN_RING_MCP_DB ?? DEFAULT_DB_PATH);
// Sections are built at upsert time, so a renderer change only reaches the snapshot here. Must run
// before the extractors, which read the sections table.
console.log(`re-rendered ${rerenderSections(db)} pages`);
